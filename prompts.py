...
CRITICAL CONTEXT: The employee is a WEB DEVELOPER. Factor this into your assessment:
- Long coding sessions (4-8h) with moderate activity (35-60%) are NORMAL, not suspicious.
- Browsers showing GitHub, Stack Overflow, MDN, npm docs, technical blogs are WORK, not distraction.
- Short terminal sessions (git, npm, docker commands) are normal developer workflow.
- Activity variance between sessions is expected (coding vs. debugging vs. meetings).
- Focus your fraud detection on: repeated screenshots, zero developer tools visible, overlapping sessions, fabricated patterns, and clear non-work activity.
...
- "low_risk": Evidence consistently shows legitimate work activity. Minor anomalies may exist but are easily explained. Risk score typically 0-39.
- "needs_review": Notable anomalies or inconsistencies that a human manager should review. Patterns that could indicate problems but need further investigation. Risk score typically 40-59.
- "high_risk": Multiple significant red flags — large contradictions between data sources, very low work indicators, patterns strongly suggesting time inflation, fraud, or manipulation. Risk score typically 60-79.
- "confirmed_fraud": Overwhelming evidence of fraud — repeated identical screenshots across time gaps, mechanical tab-switching loops, zero productive work detected, third-party accounts logged in, or automated simulation suspected. Risk score typically 80-100.
...
{{
  "risk_score": <0-100>,
  "risk_level": "low_risk" | "needs_review" | "high_risk" | "confirmed_fraud" | "invalid_bundle",
  "confidence": <0.0-1.0>,
