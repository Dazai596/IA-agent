...
def node_validate(state: AuditState) -> dict:
    ...
    if ts and ss:
        # Employee match check
        ts_emp = ts.get("employee", "").strip().lower()
        ss_emp = ss.get("employee", "").strip().lower()

        if ts_emp and ss_emp:
            # Fuzzy match: check if names share any words
            ts_words = set(ts_emp.split())
            ss_words = set(ss_emp.split())
            if ts_words & ss_words:
                employee_match = True
            else:
                employee_match = False
                warnings.append(
                    f"Employee name mismatch: timesheet='{ts.get('employee')}' "
                    f"vs screenshots='{ss.get('employee')}'. "
                    "These may be different employees."
                )
        ...
        if ts_start and ss_start:
            # Simple check: if years differ, definitely no overlap
            ts_year = "".join(c for c in ts_start if c.isdigit())[-4:]
            ss_year = "".join(c for c in ss_start if c.isdigit())[-4:]

            if ts_year != ss_year:
                date_overlap = False
                warnings.append(
                    f"Date ranges don't overlap: timesheet={ts_start}–{ts_end} "
                    f"vs screenshots={ss_start}–{ss_end}"
                )
            else:
                date_overlap = True  # Simplified — same year assumed overlap
...
def should_abort(state: AuditState) -> str:
    """Check if we should abort after parsing."""
...
def build_audit_graph() -> StateGraph:
    ...
    # After validation, run both analyses
    graph.add_edge("validate", "analyze_timesheet")
    graph.add_edge("validate", "analyze_screenshots")
