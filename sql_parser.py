...
def _detect_file_type(path: Path) -> str:
    ...
    with open(path, "rb") as f:
        header = f.read(200)
    # Real .xlsx files are ZIP archives starting with PK signature
    if header[:4] == b"PK\x03\x04" or path.suffix.lower() == ".xlsx":
        return "xlsx"
...
def _parse_html_xls(path: Path) -> TimesheetData:
    ...
    df = df[~df.apply(lambda row: row.astype(str).str.contains("Total:|\\*Average", case=False).any(), axis=1)]
