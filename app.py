...
def generate_pdf_report(report: AuditReport) -> bytes:
    """Generate a professional PDF report from the audit data."""
    ...
    class AuditPDF(FPDF):
        def header(self):
            ...
            self.cell(0, 8, "Employee Work Audit Report", align="L")
            self.cell(0, 8, report.employee, align="R", new_x="LMARGIN", new_y="NEXT")
    ...
    pdf.cell(0, 14, "Audit Report", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(100, 100, 100)
    pdf.cell(0, 7, f"{report.employee}  |  {report.date_range}", new_x="LMARGIN", new_y="NEXT")
    if report.timezone:
        pdf.cell(0, 7, f"Timezone: {report.timezone}", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 7, f"Generated: {report.generated_at.strftime('%Y-%m-%d %H:%M')}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(3)

    # ── Risk Assessment ──
    pdf.section_title("Risk Assessment")
    risk_label = get_risk_label(risk.risk_level)
    pdf.metric_row("Risk Level", risk_label)
    pdf.metric_row("Risk Score", f"{risk.risk_score:.1f} / 100")
    pdf.metric_row("Confidence", f"{risk.confidence:.0%}")
    pdf.ln(2)
    ...
    if risk.fraud_assessment:
        pdf.sub_title("Fraud Assessment")
        pdf.body_text(risk.fraud_assessment)

    if risk.score_breakdown:
        pdf.sub_title("Score Breakdown")
        for b in risk.score_breakdown:
            pdf.bullet(f"+{b.points:.0f} pts  {b.signal_name}: {b.description}")
    ...
    if report.work_summary:
        pdf.section_title("Detailed Audit Report")
        pdf.body_text(report.work_summary)

    # ── Timesheet Analysis ──
    if report.timesheet_analysis:
        ta = report.timesheet_analysis
        pdf.section_title("Timesheet Analysis")
        pdf.metric_row("Total Hours", f"{ta.total_duration_hours:.1f}h ({ta.total_sessions} sessions)")
        pdf.metric_row("Active Hours", f"{ta.total_active_hours:.1f}h ({ta.overall_activity_pct:.0f}% active)")
        pdf.metric_row("Avg Session", f"{ta.avg_session_duration_min:.0f} min ({ta.avg_activity_pct:.0f}% avg activity)")
        pdf.metric_row("Activity Range", f"{ta.min_activity_pct:.0f}% - {ta.max_activity_pct:.0f}%")
        if ta.very_short_sessions > 0:
            pdf.metric_row("Very Short Sessions", f"{ta.very_short_sessions} (< 5 min)")
        if ta.very_long_sessions > 0:
            pdf.metric_row("Very Long Sessions", f"{ta.very_long_sessions} (> 6h)")
        if ta.suspicious_hours_total != "00:00:00":
            pdf.metric_row("Suspicious Hours", f"{ta.suspicious_hours_total} ({ta.suspicious_pct:.0f}%)")

        if ta.anomalies:
            pdf.sub_title(f"Anomalies ({len(ta.anomalies)})")
            for a in ta.anomalies:
                pdf.bullet(f"[{a.severity.upper()}] {a.session_date} {a.session_time[:8]} - {a.description}")

        if ta.daily_breakdown:
            pdf.sub_title("Daily Breakdown")
            for date, hours in ta.daily_breakdown.items():
                pdf.bullet(f"{date}: {hours:.1f}h")

    # ── Screenshot Analysis ──
    if report.screenshot_analysis:
        sa = report.screenshot_analysis
        pdf.section_title("Screenshot Analysis")
        pdf.metric_row("Total Analyzed", str(sa.total_analyzed))
        pdf.metric_row("Work", f"{sa.work_count} ({sa.work_pct:.0f}%)")
        pdf.metric_row("Non-Work", f"{sa.non_work_count} ({sa.non_work_pct:.0f}%)")
        pdf.metric_row("Idle", f"{sa.idle_count} ({sa.idle_pct:.0f}%)")
        if sa.uncertain_count > 0:
            pdf.metric_row("Uncertain", str(sa.uncertain_count))

        # Classifications with thumbnails
        if sa.classifications:
            pdf.sub_title(f"Classifications ({len(sa.classifications)})")
            for c in sa.classifications:
                cat_label = c.category.value.upper().replace("_", " ")
                line = f"[{cat_label} {c.confidence:.0%}] {c.timestamp} - {c.description}"
                pdf.bullet(line)

                # Embed thumbnail if available
                if c.image_b64:
                    try:
                        img_data = b64mod.b64decode(c.image_b64)
                        img_stream = _io.BytesIO(img_data)
                        x_before = pdf.get_x()
                        pdf.image(img_stream, x=x_before + 20, w=50)
                        pdf.ln(2)
                    except Exception:
                        pass
        ...
        if sa.repeated_frames:
            pdf.sub_title(f"CRITICAL: {len(sa.repeated_frames)} Repeated Identical Frame(s)")
            for rf in sa.repeated_frames:
                pdf.bullet(
                    f"{rf.first_occurrence} -> {rf.repeat_occurrence} "
        ...
        if sa.tab_switching_analysis and sa.tab_switching_analysis.loop_detected:
            pdf.sub_title("Tab-Switching Loop Detected")
            pdf.body_text(
                f"{sa.tab_switching_analysis.loop_count} loops, "
                f"max {sa.tab_switching_analysis.max_tabs_visible} tabs visible"
            )

        # Third-party accounts
        if sa.third_party_accounts:
            pdf.sub_title(f"SECURITY: {len(sa.third_party_accounts)} Third-Party Account(s)")
            for tp in sa.third_party_accounts:
                pdf.bullet(f"{tp.timestamp}: Found {tp.email_found} (expected: {tp.expected_email})")

        # Unauthorized access
        if sa.unauthorized_access_events:
            pdf.sub_title(f"Unauthorized Access ({len(sa.unauthorized_access_events)})")
            for ua in sa.unauthorized_access_events:
                pdf.bullet(f"{ua.timestamp}: {ua.domain}")

        # Suspicious sites
        if sa.suspicious_sites:
            pdf.sub_title(f"Suspicious Sites ({len(sa.suspicious_sites)})")
            for ss_site in sa.suspicious_sites:
                pdf.bullet(f"{ss_site.timestamp}: {ss_site.site_name} - {ss_site.reason}")

        # Monitor inconsistencies
        if sa.monitor_inconsistencies:
            pdf.sub_title(f"Monitor Inconsistencies ({len(sa.monitor_inconsistencies)})")
            for mi in sa.monitor_inconsistencies:
                pdf.bullet(
                    f"{mi.date}: {mi.single_monitor_count} single + "
                    f"{mi.dual_monitor_count} dual-monitor screenshots"
                )

    # ── Suspicious Hours ──
    if report.timesheet_analysis and report.timesheet_analysis.suspicious_windows:
        ta_s = report.timesheet_analysis
        pdf.section_title("Suspicious Hours")
        pdf.metric_row("Suspicious Time", f"{ta_s.suspicious_hours_total} ({ta_s.suspicious_pct:.0f}%)")
        pdf.metric_row("Windows Flagged", str(len(ta_s.suspicious_windows)))
        for sw in ta_s.suspicious_windows:
            pdf.bullet(f"{sw.start} - {sw.duration} - {sw.reason}")

    # ── Cross Analysis ──
    if report.cross_analysis:
        ca = report.cross_analysis
        pdf.section_title("Cross Analysis")
        pdf.metric_row("Timesheet Activity", f"{ca.timesheet_activity_pct:.0f}%")
        pdf.metric_row("Screenshot Work", f"{ca.screenshot_work_pct:.0f}%")
        pdf.metric_row("Activity Gap", f"{ca.activity_gap:.0f} percentage points")

        if ca.contradictions:
            pdf.sub_title("Contradictions")
            for item in ca.contradictions:
                pdf.bullet(item)
        if ca.consistencies:
            pdf.sub_title("Consistencies")
            for item in ca.consistencies:
                pdf.bullet(item)
        if ca.reasoning:
            pdf.sub_title("AI Cross-Analysis Reasoning")
            pdf.body_text(ca.reasoning)

    # ── Validation ──
    if report.validation and (report.validation.errors or report.validation.warnings):
        pdf.section_title("Validation")
        for e in report.validation.errors:
            pdf.bullet(f"[ERROR] {e}")
        for w in report.validation.warnings:
            pdf.bullet(f"[WARNING] {w}")

    # ── Facts & Interpretations ──
    if risk.facts or risk.interpretations:
        pdf.section_title("Facts & Interpretations")
        if risk.facts:
            pdf.sub_title("Facts")
            for f in risk.facts:
                pdf.bullet(f)
        if risk.interpretations:
            pdf.sub_title("Interpretations")
            for i in risk.interpretations:
                pdf.bullet(i)

    # ── AI Reasoning ──
    if risk.reasoning:
        pdf.section_title("Full AI Reasoning")
        pdf.body_text(risk.reasoning)

    # ── Session Reports ──
    if report.session_reports:
        pdf.section_title("Session-by-Session Reports")
        for sr in report.session_reports:
            verdict_label = sr.session_verdict.upper().replace("_", " ")
            pdf.sub_title(
                f"{sr.session_date} | {sr.duration} ({sr.duration_hours:.1f}h) | "
                f"{sr.screenshots_in_session} screenshots | {verdict_label} | Score: {sr.session_risk_score:.0f}"
            )
            if sr.findings:
                for f in sr.findings:
                    pdf.bullet(f"[{f.severity.upper()}] {f.timestamp} {f.finding_type}: {f.description}")
            else:
                pdf.body_text("No findings - session appears legitimate.")
