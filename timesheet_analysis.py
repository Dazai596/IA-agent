...
def _compute_daily_breakdown(sessions: list[WorkSession]) -> dict[str, float]:
    ...
    for s in sessions:
        daily[s.date_start] += float(s.duration_minutes) / 60
...
def _compute_duckdb_stats(sessions: list[WorkSession]) -> dict:
    ...
    for s in sessions:
        records.append({
            "date": s.date_start,
            "duration_min": float(s.duration_minutes),
            "active_min": float(s.active_minutes),
            "idle_min": float(s.idle_minutes),
            "activity_pct": float(s.activity_pct),
            "start_hour": int(_parse_time_hour(s.time_start)),
            "start_minutes": int(_parse_time_minutes(s.time_start)),
        })
    ...
    # Round duration check — use tolerance because float modulo is imprecise
    # (e.g. 60.000001 % 30 ≈ 0.000001, not exactly 0)
    if total_sessions > 0:
        round_count = ((df["duration_min"] > 0) & ((df["duration_min"] % 30).round(1) == 0)).sum()
...
def _compute_suspicious_windows(
    ...
    for s in sessions:
        ...
        _dur_min = float(s.duration_minutes)
        _idle_min = float(s.idle_minutes)
        idle_ratio = safe_divide(_idle_min, _dur_min)
        settings = get_settings()
        if _dur_min > 30 and idle_ratio > settings.idle_ratio_threshold:
            suspicious_seconds = _idle_min * 60
            ...
            windows.append(SuspiciousWindow(
                ...
                reason=f"High idle ratio ({idle_ratio:.0%}) — {_idle_min:.0f} min idle in {_dur_min:.0f} min session",
                session_date=s.date_start,
            ))

        # Very long sessions (>6h) — the entire excess beyond 6h is suspicious
        settings = get_settings()
        if _dur_min > settings.very_long_session_min:
            excess_min = _dur_min - settings.very_long_session_min
            ...
            windows.append(SuspiciousWindow(
                ...
                reason=f"Very long session ({_dur_min:.0f} min / {_dur_min/60:.1f}h) — excess time beyond {settings.very_long_session_min/60:.0f}h",
                session_date=s.date_start,
            ))
