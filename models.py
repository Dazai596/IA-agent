...
class ScreenshotClassification(BaseModel):
    ...
    ocr_text: str = Field(default="", repr=False)  # OCR text extracted from this screenshot
...
class SessionScreenshotMatch(BaseModel):
    """Maps one timesheet work session to the screenshots taken during it."""
    session_date: str
    session_start: str
    session_end: str
...
class TemporalGap(BaseModel):
    """A suspicious temporal pattern where claimed work and observed evidence conflict."""
    session_date: str
    session_start: str
    session_end: str
    gap_type: str  # "non_work_during_high_activity" | "zero_work_in_session" | "high_activity_no_screenshots"
    timesheet_activity_pct: float = 0.0
...
class CrossAnalysisResult(BaseModel):
    ...
    reasoning: str = ""
    # Session-level temporal evidence (new)
    session_screenshot_matches: list[SessionScreenshotMatch] = Field(default_factory=list)
    temporal_gaps: list[TemporalGap] = Field(default_factory=list)
    temporal_contradictions: list[str] = Field(default_factory=list)
    # Sequential pattern analysis (new)
    sequential_pattern: str = ""        # overall_pattern from sequential analysis
    suspicious_clusters: list[dict] = Field(default_factory=list)
...
class DayWorkPattern(BaseModel):
    """Per-day work pattern for the structured report."""
    date: str
...
class StructuredReportSections(BaseModel):
    """Structured sections for the professional activity report PDF."""
    strengths: list[str] = Field(default_factory=list)
    consistency_indicators: list[str] = Field(default_factory=list)
    work_style_observations: list[str] = Field(default_factory=list)
    overall_efficiency_assessment: str = ""
    performance_overview: str = ""
    daily_patterns: list[DayWorkPattern] = Field(default_factory=list)
    suspicious_sessions: list[dict] = Field(default_factory=list)
...
class AuditReport(BaseModel):
    ...
    date_range: str
    timezone: str = ""
    department: str = "developer"   # developer | backoffice | telemarketing
    ...
    work_summary: str = ""
    structured_sections: Optional[StructuredReportSections] = None
